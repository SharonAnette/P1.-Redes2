...
int main() {
    ...
    while (1) {
        ...
        // AÃ±adir el socket del servidor al conjunto de descriptores
        FD_SET(server_fd, &readfds);
        max_sd = server_fd;

        // AÃ±adir los sockets de cliente al conjunto de descriptores
        ...
        // Si hay una actividad en el socket del servidor, es una nueva conexiÃ³n
        if (FD_ISSET(server_fd, &readfds)) {
            ...
            printf("Nueva conexiÃ³n, socket fd es %d, ip es : %s, puerto : %d\n",
                   new_socket, inet_ntoa(address.sin_addr), ntohs(address.sin_port));

            // AÃ±adir el nuevo socket al array de sockets de cliente
            for (int i = 0; i < max_clients; i++) {
                if (client_socket[i] == 0) {
                    client_socket[i] = new_socket;
                    printf("AÃ±adiendo a la lista de sockets como %d\n", i);
                    break;
                }
            }
        }
        ...
        for (int i = 0; i < max_clients; i++) {
            ...
            if (FD_ISSET(sd, &readfds)) {
                ...
                if ((valread = read(sd, buffer, BUFFER_SIZE)) == 0) {
                    // Alguien se desconectÃ³, obtener detalles e imprimir
